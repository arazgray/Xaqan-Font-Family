    ...
    def extract_three_dots(self):
            ...
            if components:
                ...
                for i in range(3):
                    if i < len(components):
                        ...
                        dot_glyph.addReference(ref[0], ref[1])
        ...
        try:
            ...
            for i in range(3):
                ...
                pen.curveTo((0, dot_size/2), (dot_size/2, 0), (0, -dot_size/2))
                pen.closePath()
                pen = None
    ...
    def add_gsub_features(self):
        ...
        try:
            ...
            try:
                ...
                if not hasattr(self.font, 'gsub'):
                    self.font.addLookup('gsub', 'gsub',
                                      [('arab', ['dflt'])])

                # Add lookups for each feature
                for feature_name in ['init', 'medi', 'fina']:
                    # Create lookup for this feature
                    lookup_name = f"xaqan_{feature_name}"
                    self.font.addLookup(lookup_name, 'gsub',
                                      [('arab', ['dflt'])])

                    # Add substitution rules
                    if feature_name == 'init':
                        self.font.addSubstitution(lookup_name, 'uni063D',
                                                'uni063D.init')
                        self.font.addSubstitution(lookup_name, 'uni0763',
                                                'uni0763.init')
                        self.font.addSubstitution(lookup_name, 'uni06AD',
                                                'uni06AD.init')
                    elif feature_name == 'medi':
                        self.font.addSubstitution(lookup_name, 'uni063D',
                                                'uni063D.medi')
                        self.font.addSubstitution(lookup_name, 'uni0763',
                                                'uni0763.medi')
                ...
                try:
                    ...
                    self.font.parseFeatureFile(feature_data)
    ...
    def add_gpos_feature(self):
        ...
        try:
            ...
            try:
                ...
                self.font.addLookup(lookup_name, 'gpos',
                                  [('arab', ['dflt'])])
        ...
        try:
            ...
            try:
                ...
                if hasattr(self.font, 'sfnt_names'):
                    ...
                    new_names = []
                    for name_id, value in existing_names:
                        if name_id == 1:  # Family name
                            new_names.append((name_id, new_family_name))
                        elif name_id == 2:  # Style name
                            # Preserve style name
                            new_names.append((name_id, value))
                        elif name_id == 4:  # Full name
                            new_names.append((name_id, new_fullname))
                        elif name_id == 6:  # PostScript name
                            new_names.append((name_id, new_fontname))
                        else:
